---
name: motion
description: Use when building React or Next.js interfaces with complex interactive animation, gestures, scroll-linked effects, shared layout transitions, spring physics, SVG animation, or Motion-specific bundle optimization needs.
---
...
Control how multiple child animations interact:

```tsx
import { AnimatePresence } from "motion/react"

// Default: all children animate out simultaneously (sync)
...
// Wait: children animate out completely before new ones animate in
...
// popLayout: exiting element shrinks out, then entering element pops in
// Great for accordion-style expand/collapse with layout animations
...
|------|----------|----------|
| `sync` (default) | All animations play simultaneously | Concurrent exits/entries |
| `wait` | Exit completes before enter starts | Sequential transitions |
| `popLayout` | Exit shrinks (scale down) before enter pops (scale up) | Accordion, expandable panels |
...
### 8. MotionValue and useMotionValue

`MotionValue` is a reactive value container that tracks animation state without causing React re-renders. Use when you need fine-grained control over animation values.

#### useMotionValue

Creates a MotionValue that can be used in motion component styles:
...
**Key behaviors:**
- Updates without triggering React re-renders (works outside render cycle)
- Can be passed directly to `style` prop of motion components
- Composable with `useTransform`, `useSpring`, `useTime`

#### useMotionValueEvent

Subscribe to MotionValue changes without creating a reactive context:
...
**Events available:** `"change"`, `"animationStart"`, `"animationComplete"`, `"animationCancel"`

#### useSpring

Standalone spring animation for any value (not just motion component props):
...
#### useTransform with Function Input

Map a value through multiple ranges:
...
function AdvancedTransform() {
  ...
  const background = useTransform(
    x,
    [0, 50, 100],        // input ranges
    ["#fff", "#ff0", "#f00"]  // output values
  )

  return <motion.div style={{ backgroundColor: background }} />
}
```
...
Visual debugger for Motion animations:

**Installation**: [Motion DevTools - Chrome Web Store](https://chromewebstore.google.com/detail/motion/nidganghegonkcecgjgpppihfknjobec)

**Features:**
- Inspect all MotionValues in real-time
- Scrub through animation timelines
- See component props and state
- Highlight motion components in the page

**Usage:**
1. Install the Chrome extension
2. Open DevTools (F12) → Motion tab
3. Interact with your app — all motion components appear in the panel
4. Click any component to inspect its MotionValues and animation state

**Note**: DevTools requires Motion v11+ with the development build.
